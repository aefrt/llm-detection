...
3. Попробовал в `dataset.ipynb` через Kaggle Notebooks аугмееентацию с помощью `API Groq`. Минут за 20 нагенерировал 200 текстов, а дальше токены закончились. Можно ли как-то ускорить процесс: использовать что-то другое, например? 
4. Посмотрел на тексты с пересечением в `eda.ipynb`. В нем особо выделялись слова `help, life` у LLM, в то время как у людей чисто нейтральные глаголы и артикли. Выяснилось, что LLM просто везде любит пихать определенный оборот. Вроде явно на конкретной теме не фокусируется.
...
7. Провел доп эксперименты с `tf-IDF, SentenceTransformers` в `model.ipynb`, все ли ок? Почему-то `optuna` не смогла построить что-то, кроме иф-елс пней для катбуста.
